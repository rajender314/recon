...
@Component({
  ...
  styleUrls: ['../users.component.scss', './clients.component.scss']
})
export class ClientsComponent implements OnInit {
  ...
  public state = {
    ...
    selectedTab: { id: 2, label: "Clients", displayLabel: "Clients", route: "users/clients" },
    selectedItem: {
      logo: '',
      name: '',
      status: false,
    ...
    detailTabs: [
      ...
      { label: "Job Access", type: 2, method: 'job_access' },
  ...
  getDropDownMaster = () => {
    this.contactsService.getDropdownMaster({ org_type: 2, status: true }).then(response => {
  ...
  saveRoleUsers(form: any): void {
    ...
    if (form.valid) {
      ...
      if (method == 'external_apps') {
        params['id'] = this.state.selectedItem['contact_id'];
        params['org_type'] = 2;
        ...
        params['id'] = this.state.selectedItem['job_access_id'];
        ...
        this.usersService.saveClientJobAccess(params).then(response => {
          if (response.result.success) {
            this.snackbar.openFromComponent(SnackbarComponent, {
              data: { status: 'success', msg: 'Permissions Updated Successfully' },
              verticalPosition: 'top',
  ...
  getPermissions = method => {
    if (method == 'external_apps') {
      this.state.detailsLoader = true;
      this.usersService.getExternalApps({ id: this.state.selectedItem['contact_id'], org_type: 2 })
        .then(res => {
      ...
      this.state.detailsLoader = true;
      this.usersService.getPermissions({ method: method, org_type: 2 })
        .then(res => {
          if (res.result.success) {
            this.state.permissions[method] = res.result.data['items'];
            if (this.state.permissions[method] && this.state.permissions[method][0]) {
              this.createForm();
              this.state.detailsLoader = false;
              this.roleUsers();
            } else {
  ...
  roleUsers(): void {
    this.usersService.getClientJobAccess({ method: this.state.activeTab['method'], user_id: this.state.selectedItem['contact_id'], org_type: 2 }).then(response => {
      if (response.result.success) {
        this.state.selectedItem['job_access_id'] = response.result.data.id;
      ...
      this.usersService.getUserData({
        ...
        if (response.result.success) {
          ...
          if (this.state.activeTab.type == 0) {
            this.createForm();
          } else if (this.state.activeTab.type == 2 || this.state.activeTab.type == 5) {
  ...
  createForm(): void {
    ...
    if (!this.state.activeTab['method']) {
      ...
      this.ivieForm.removeControl(method);
...
phonenumber(event: any) {
  ...
  save(form, section): void {
    this.submitted = true;
    if (form.valid) {
      this.usersService.saveUser({...form.value, ...{org_type: 2}})
        .then(response => {
          if (response.result.success) {
            this.snackbar.openFromComponent(SnackbarComponent, {
  ...
  addUser(): void {
    this.dialogRef = this.dialog.open(AddNewUserComponent, {
      ...
      data: {
        title: "Add User",
        searchForm: [
          { name: "company", type: "select", required: true, label: "Client Company", value: "", options: "companies" },
